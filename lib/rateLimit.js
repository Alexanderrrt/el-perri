...
/**
 * Rate limiter for the in-app AI assistant (/api/assistant) — per IP.
 * Each chat turn hits the paid LLM, so this bounds cost/abuse while staying
 * generous enough for a real ordering conversation.
 * 40 messages per hour per IP.
 */
export function createChatLimiter() {
  return (request) => {
    const ip = request.headers.get("x-forwarded-for") || "unknown";
    return checkRateLimit(`chat:${ip}`, 40, 60 * 60 * 1000);
  };
}
