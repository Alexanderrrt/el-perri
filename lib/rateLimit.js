...
/**
 * Rate limiter for the WhatsApp AI bot — keyed by the sender's phone number
 * (not IP, since every request originates from Meta's servers). Generous
 * enough for a real ordering conversation, tight enough to bound DeepSeek +
 * WhatsApp API cost from a single abusive number.
 * 30 messages per hour per phone number.
 */
export function createWhatsAppLimiter() {
  return (phone) => checkRateLimit(`whatsapp:${phone}`, 30, 60 * 60 * 1000);
}
