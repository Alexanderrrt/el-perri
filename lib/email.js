...
/**
 * Catering lead → owner's inbox (CATERING_EMAIL). Plain-text body so
 * user-submitted content can't inject HTML. No-ops when email isn't
 * configured; the API route then tells the client to offer WhatsApp instead.
 */
export async function sendCateringLead(lead) {
  const to = process.env.CATERING_EMAIL;
  if (!emailConfigured || !to) {
    console.log(`[EMAIL] (skipped — missing ${emailConfigured ? 'CATERING_EMAIL' : 'RESEND_API_KEY'}) catering lead from ${lead.name}`);
    return { skipped: true };
  }
  try {
    const result = await resend.emails.send({
      from: FROM,
      to,
      subject: `🎉 Nueva cotización de catering — ${lead.name}`,
      text: [
        'Nueva solicitud de catering desde la web:',
        '',
        `Nombre:    ${lead.name}`,
        `Teléfono:  ${lead.phone}`,
        `Fecha:     ${lead.date || '—'}`,
    ...
    console.error(`[EMAIL] catering lead failed:`, error.message);
    return { error: error.message };
  }
}
